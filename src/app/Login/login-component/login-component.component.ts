 ...
 loadProviders() {
  ...
  this.loginService.getAuthProviders().subscribe((res: any) => {
    ...
    if (this.providers.length > 0) {
      ...
      const firstOrder = this.providers[0].order;

      // check if multiple providers share same order
      this.currentProviders = this.providers.filter(p => p.order === firstOrder);

      if (this.currentProviders.length > 1) {

        // SINGLE FACTOR → show all
        this.currentProvider = null;

      } else {

        // existing logic for 2FA / MFA
      ...
      this.messageService.add({
        ...
        detail: 'No authentication providers found (or none active)'
      });
    ...
    this.messageService.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Failed to load auth providers'
    });
